...
public class 네이버경제뉴스 {
   ...
   public static void main(String[] args) throws IOException {
		...
		for (int i = 1; i < pages; i++) { // 0페이지 = 1페이지. 1페이지부터 쓰기
			...
			for (int j = 0; j < photoelements.size(); j++) {
				...
				Elements a_newelements = newelements.select("a"); // 기사에 대한 url

				Element a_newelement = a_newelements.get(0);

				// System.out.println(a_newelement);

				String newUrl = a_newelement.attr("href"); // 기사링크 //세부 url
				...
				Element contentemement = detailDoc.getElementById("dic_area"); // 상세 기사 내용
