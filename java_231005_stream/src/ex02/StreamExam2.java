...
public class StreamExam2 {
	public static void main(String[] args) {
		List<PriceInfo> lists = new ArrayList<PriceInfo>();	//초기 생성시 16칸 생성. 값 추가시 주소값을 참조시킴. generic 을 이용해 특정 객체를 이용해 생성한 값을 주입하도록 제한할 수 있음
		...
		int sum = lists.stream().filter(p->p.getPrice()>500).mapToInt(t->t.getPrice()).sum();
										//model, price 가 통과됨	//price 만 출력, int를 받을 때
		System.out.println(sum);
		
		System.out.println("================================");
		//0.List 데이터 3개 추가
		//1.PriceInfo 클래스 멤버변수 model getter 생성
		//2.필터링 조건 : model 이 참조하는 문자열 길이가 4이상이면 해당 인스턴스 통과
